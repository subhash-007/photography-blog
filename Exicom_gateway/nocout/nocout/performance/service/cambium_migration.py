...
def main():
    ...
    sql_db=configs.get(mongo_conf[0][0]).get('sql_db'), table_name=configs.get(mongo_conf[0][0]).get('cambium_topology').get('table_name'), ip=configs.get(mongo_conf[0][0]).get('ip')
    )
